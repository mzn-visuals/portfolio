// LiquidEther - Direct browser compatible version
// This is a vanilla JS implementation that doesn't require JSX compilation
...
class LiquidEther {
  constructor(container, options = {}) {
    ...
    this.mountRef = { current: container };
...
`;
    ...
    const manager = {
      ...
      loop: function() {
        ...
        this.render();
        this.rafRef = requestAnimationFrame(() => this.loop());
      },
    ...
    manager.init();
    this.webglRef.current = manager;
    
    // Setup resize handler
    manager._resize = () => manager.resize();
    window.addEventListener('resize', manager._resize);
    
    // Setup visibility handler
    manager._onVisibility = () => {
    ...
    document.addEventListener('visibilitychange', manager._onVisibility);
  ...
  dispose() {
    const manager = this.webglRef.current;
    if (manager) {
      manager.dispose();
    }
